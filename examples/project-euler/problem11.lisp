...
(from:string->array char:new-line)
(array:map (lambda line (|> line 
                            (from:string->array char:space) 
                            (array:map from:string->integer))))))

(let matrix:top-down [[0 0] [1 0] [-1 0] [2 0]])
(let matrix:left-right [[0 0] [0 -1] [0 1] [0 2]])
(let matrix:left-diag [[0 0] [-1 -1] [1 1] [2 2]])
(let matrix:right-diag [[0 0] [1 -1] [-1 1] [-2 2]])
        ...
        (math:var-set! max (math:max (math:var-get max) (matrix:adjacent-product grid matrix:top-down y x *)))
         (math:var-set! max (math:max (math:var-get max) (matrix:adjacent-product grid matrix:left-right y x *)))
          (math:var-set! max (math:max (math:var-get max) (matrix:adjacent-product grid matrix:left-diag y x *)))
           (math:var-set! max (math:max (math:var-get max) (matrix:adjacent-product grid matrix:right-diag y x *))))))))
(math:var-get max)