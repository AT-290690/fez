...
(let part1 (lambda input (do
    ...
    (let disk (|> input
        (reducing-enumerated:number->array (lambda disk ch i 
        (array:merge! disk
            (if (math:even? i) (do
                    (let id (var:get (var:increment! file-id)))
                    (array:of ch (lambda . id)))
                    (array:of ch (lambda . -1))))) [])))
    (let blanks (reducing-enumerated:number->array disk (lambda a x i (if (= x -1) (array:append! a i) a)) []))
    (let recursive:fragment (lambda ind (do
        ...
        (|> disk (reducing-enumerated:number->number  (lambda a b i (+ a (* b i))) 0)))))
