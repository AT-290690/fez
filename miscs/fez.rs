...
/// Transform (- a b c ...) into nested (- (- a b) c ...)
/// Transform (- x) into (* x -1)
fn minus_transform(mut exprs: Vec<Expression>) -> Expression {
    // exprs[0] is "-"
    exprs.remove(0);

    match exprs.len() {
        0 => Expression::Atom(0.0), // just (-) → 0
        1 => {
            // unary minus: (- x) → (* x -1)
            Expression::Apply(vec![
                Expression::Word("*".to_string()),
...
/// Transform (cond test1 expr1 test2 expr2 ... default)
/// into nested (if test1 expr1 (if test2 expr2 ... default))
fn cond_transform(mut exprs: Vec<Expression>) -> Expression {
    // exprs[0] is "cond"
    exprs.remove(0);

    if exprs.is_empty() {
        return Expression::Atom(0.0); // empty cond => 0
    }
...
/// Transform (|> inp (f a b) (g c)) into nested calls.
fn pipe_transform(mut exprs: Vec<Expression>) -> Expression {
    // exprs[0] is the symbol "|>", exprs[1..] are rest
    let mut inp = exprs.remove(1); // first argument to pipe
    ...
    for stage in exprs.into_iter().skip(1) {
        if let Expression::Apply(mut inner) = stage {
            ...
            // stage is just a function symbol with no args
...
fn is_number(s: &str) -> bool {
    // Accept: 123, -123, 1.23, -1.23, .5, -.5, 5., -5.
    // Reject: '-', '.', '+', '+1' (treat '+' as symbol)
    // Simple check first:
    if s == "-" || s == "." || s == "+ " || s == "+" {
        return false;
    }
    ...
    // Remaining must be digits and at most one dot
